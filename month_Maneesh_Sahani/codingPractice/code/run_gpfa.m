% EXE This is a modified version of the example.m file that is
% distributed with the GPFA code set.  Most of the comments are
% original.  Changes are marked with %EXE%

% Note that this code uses X for latent and Y for observed variable.
...
% include gpfa paths
addpath gpfa
addpath gpfa/core_gpfa
addpath gpfa/core_twostage
addpath gpfa/plotting
addpath gpfa/util
addpath gpfa/util/precomp
addpath gpfa/util/invToeplitz
...
dat = struct(...
    'trialId', num2cell([1:nTrial]'), ...
    'spikes', squeeze(num2cell(XX,1:2))...
    ); 

%% EXE:
% Set some arguments controlling timescales. GPFA code expects
% spiketrains binned at 1ms; we have counts at 20ms.  So we don't bin
% any further.
dataArgs = {'binWidth', 1, 'segLength', 50, 'startTau', 5};
...
result = neuralTraj(runIdx, dat, 'method', method, dataArgs{:},...
                    'xDim', xDim, 'kernSDList', kernSD);
...
for xDim = [2 5 8]
  neuralTraj(runIdx, dat, 'method',  'pca', 'xDim', xDim, dataArgs{:}, ...
             'numFolds', numFolds);
  neuralTraj(runIdx, dat, 'method', 'ppca', 'xDim', xDim, dataArgs{:}, ...
             'numFolds', numFolds);
  neuralTraj(runIdx, dat, 'method',   'fa', 'xDim', xDim, dataArgs{:}, ...
             'numFolds', numFolds);
  neuralTraj(runIdx, dat, 'method', 'gpfa', 'xDim', xDim, dataArgs{:}, ...
             'numFolds', numFolds);
end
fprintf('\n');
% NOTES:
...
kernSD = 5; % select kernSD for two-stage methods
